...
import { getMe, getPatient, listStaff, updatePatient } from '@/lib/api'
...
export default function EditPatientPage() {
  ...
  const hnParam = params.hn as string
  ...
  useEffect(() => {
    ...
    getMe().then(u => {
      if (!u.roles?.includes('edit_case')) { router.push('/login'); return }
    }).catch(() => router.push('/login'))
  }, [])

  useEffect(() => {
    if (!hnParam) return
    getPatient(hnParam).then(p => {
      setEditingLocked(Boolean(p.line_user_id))
      setForm(f => ({
        ...f,
        hn: p.hn,
        ...
        surgery_date: p.extra_info?.surgery_date || '',
        discharge_date: p.extra_info?.discharge_date || '',
        note: p.extra_info?.note || '',
        imf_wire: p.extra_info?.imf_wire ?? false,
        imf_wire_loops: p.extra_info?.imf_wire_loops?.toString() ?? '',
        imf_elastic: p.extra_info?.imf_elastic ?? false,
        imf_elastic_loops: p.extra_info?.imf_elastic_loops?.toString() ?? '',
        special_icbg: p.extra_info?.special_icbg ?? false,
        special_icbg_description: p.extra_info?.special_icbg_description || '',
        special_ng_tube: p.extra_info?.special_ng_tube ?? false,
        special_ng_tube_description: p.extra_info?.special_ng_tube_description || '',
        surgical_tooth_numbers: p.extra_info?.surgical_tooth_numbers || '',
        extraction_tooth_numbers: p.extra_info?.extraction_tooth_numbers || '',
      }))
      if (p.procedures) setSelectedProcedures(p.procedures)
      if (p.extra_info?.lefort_sub_options) setLefortSub(p.extra_info.lefort_sub_options)
      if (p.extra_info?.bssro_sub_options) setBssroSub(p.extra_info.bssro_sub_options)
      if (p.extra_info?.biopsy_sub_options) setBiopsySub(p.extra_info.biopsy_sub_options)
  ...
  if (initialLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50"><Spinner className="w-8 h-8" /></div>
  }
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input className={`input ${fieldErrors.first_name ? 'border-red-300 focus:border-red-400' : ''}`} value={form.first_name} onChange={e => set('first_name', e.target.value)} required disabled={editingLocked} />
              ...
              <div>
                ...
                <input className={`input ${fieldErrors.last_name ? 'border-red-300 focus:border-red-400' : ''}`} value={form.last_name} onChange={e => set('last_name', e.target.value)} required disabled={editingLocked} />
              ...
              <div>
                ...
                <input type="date" className="input" value={form.date_of_birth} disabled={editingLocked}
              ...
              <div>
                ...
                <input type="date" className="input" value={form.surgery_date} disabled={editingLocked}
              ...
              <div>
                ...
                <input type="date" className={`input ${fieldErrors.discharge_date ? 'border-red-300 focus:border-red-400' : ''}`} value={form.discharge_date} disabled={editingLocked}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    required
                  />
                  {nurseOpen && staff.length > 0 && !editingLocked && (
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {PROCEDURE_OPTIONS.map(p => {
                ...
                const isSurgicalRemoval = p.includes('ผ่าตัดถอนฟัน')
                const isExtraction = p.includes('ถอนฟัน') && !isSurgicalRemoval
                const checked = selectedProcedures.includes(p)
                return (
                  <div key={p}>
                    <label className="flex items-start gap-2 cursor-pointer">
                      <input type="checkbox" className="accent-primary w-4 h-4 mt-0.5" checked={checked} onChange={() => toggleProcedure(p)} disabled={editingLocked} />
                      <span className="text-sm text-gray-700">{p}</span>
                    </label>
                    {/* Lefort I sub-options */}
                    {isLefort && checked && (
                      <div className="ml-6 mt-2 flex flex-wrap gap-2">
                        {['Advancement', 'Setback', 'Osteotomy', '2 pieces', 'Impaction'].map(opt => (
                          <label key={opt} className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs border cursor-pointer transition-colors ${lefortSub.includes(opt) ? 'bg-blue-100 border-blue-400 text-blue-800' : 'bg-white border-blue-200 text-gray-600 hover:bg-blue-50'}`}>
                            <input type="checkbox" className="sr-only" checked={lefortSub.includes(opt)} onChange={() => toggleSub(lefortSub, setLefortSub, opt)} disabled={editingLocked} />
                            {opt}
                          </label>
                        ))}
                      </div>
                      ...
                      <div className="ml-6 mt-2 flex flex-wrap gap-2">
                        {['Setback', 'Advancement'].map(opt => (
                          <label key={opt} className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs border cursor-pointer transition-colors ${bssroSub.includes(opt) ? 'bg-green-100 border-green-400 text-green-800' : 'bg-white border-green-200 text-gray-600 hover:bg-green-50'}`}>
                            <input type="checkbox" className="sr-only" checked={bssroSub.includes(opt)} onChange={() => toggleSub(bssroSub, setBssroSub, opt)} disabled={editingLocked} />
                            {opt}
                          </label>
                        ))}
                      </div>
                      ...
                      <div className="ml-6 mt-2 flex flex-wrap gap-2">
                        {['Excisional', 'Incisional'].map(opt => (
                          <label key={opt} className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs border cursor-pointer transition-colors ${biopsySub.includes(opt) ? 'bg-pink-100 border-pink-400 text-pink-800' : 'bg-white border-pink-200 text-gray-600 hover:bg-pink-50'}`}>
                            <input type="checkbox" className="sr-only" checked={biopsySub.includes(opt)} onChange={() => toggleSub(biopsySub, setBiopsySub, opt)} disabled={editingLocked} />
                            {opt}
                          </label>
                        ))}
                      </div>
                    )}
                    {/* Surgical tooth number */}
                    {isSurgicalRemoval && checked && (
                      <div className="ml-6 mt-2">
                        <input className="input w-48 text-sm" placeholder="หมายเลขซี่ฟัน เช่น 18, 38" value={form.surgical_tooth_numbers}
                          onChange={e => set('surgical_tooth_numbers', e.target.value.replace(/[^0-9,\s]/g, ''))} disabled={editingLocked} />
                      </div>
                    )}
                    {isExtraction && checked && (
                      <div className="ml-6 mt-2">
                        <input className="input w-48 text-sm" placeholder="หมายเลขซี่ฟัน เช่น 18, 38" value={form.extraction_tooth_numbers}
                          onChange={e => set('extraction_tooth_numbers', e.target.value.replace(/[^0-9,\s]/g, ''))} disabled={editingLocked} />
                      </div>
                    )}
                  </div>
                )
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                {form.imf_wire && <input className="input" type="number" min={1} max={10} placeholder="จำนวน loop (1–10)" value={form.imf_wire_loops} onChange={e => set('imf_wire_loops', e.target.value)} />}
              </div>
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
              ...
              <div className="space-y-2">
                ...
                {form.imf_elastic && <input className="input" type="number" min={1} max={10} placeholder="จำนวน loop (1–10)" value={form.imf_elastic_loops} onChange={e => set('imf_elastic_loops', e.target.value)} />}
              </div>
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={e => {
                      ...
                      // trim selected dates if over new limit
